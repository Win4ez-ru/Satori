"""Offline plan, runtime-shape and fail-closed tests for Checkpoint 14.3 A/B attempt 2."""
...
from tests.checkpoint143_openai_ab_attempt2_manual_eval import (
    AUTHORIZATION_ID,
    ...
    CellLedger,
    Checkpoint143ABAttempt2ConfigurationError,
    ExecutionPlan,
...
def _fake_report() -> dict[str, Any]:
    ...
    report: dict[str, Any] = {
        ...
        "artifact_id": "checkpoint143-ab-attempt2-fixture",
                    ...
                    try:
                        reply = await runtime.services.talk.execute(
                            TalkInput(
                                ...
                                trace_id=f"attempt2-{label}-trace-{turn}",
                                client_request_id=f"attempt2-{label}-request-{turn}",
...
def test_exact_preflight_rejects_authority_call_cost_and_digest_drift() -> None:
    ...
    with pytest.raises(Checkpoint143ABAttempt2ConfigurationError):
        _preflight_shape(
            execute=True,
...
def test_exact_preflight_rejects_false_installed_wheel_parity(
    ...
    monkeypatch.setattr(
        "tests.checkpoint143_openai_ab_attempt2_manual_eval.ExecutionPlan",
        DriftedPlan,
    )
    with pytest.raises(Checkpoint143ABAttempt2ConfigurationError, match="wheel/source parity"):
        _preflight_shape(
            execute=True,
            authorization_id=AUTHORIZATION_ID,
            maximum_provider_calls=MAXIMUM_PROVIDER_CALLS,
...
def test_review_writers_freeze_phase_one_before_reveal_and_require_frozen_inputs(
    ...
    phase_1_path = (
        evaluation_root
        / "evaluations"
        / ("checkpoint143-openai-v27-v28-ab2-2026-08-31.phase1.json")
    )
    phase_2_path = (
        evaluation_root
        / "evaluations"
        / ("checkpoint143-openai-v27-v28-ab2-2026-08-31.phase2-template.json")
    )
    ...
    final_path = (
        evaluation_root
        / "evaluations"
        / ("checkpoint143-openai-v27-v28-ab2-2026-08-31.review.json")
    )
    assert finalized["accepted"] is True
